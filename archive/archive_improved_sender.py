...
def send_email(sender_email, sender_password, smtp_server, smtp_port, recipient_email, paper_title):
    ...
    <html>
      <body>
        ...
        Registering and presenting is WITHOUT CHARGE for researchers that are receiving this invitation. This is a <strong>virtual conference</strong>, so presenters will submit a 10-15 minute recording of their keynote research presentation. Attendance in the live conference is highly encouraged in order to answer audience questions, but not mandatory.<br><br>
...
def send_emails_from_csv(sender_emails, sender_password, smtp_server, smtp_port, csv_filename):
    email_index = 0 
    email_count = 0
    with open(csv_filename, 'r', newline='', encoding = 'utf-8') as csvfile:
        ...
        for row in reader:
            ...
            sender_email = sender_emails[email_index]
            retries = 2  
            while retries > 0:
                try:
                    send_email(sender_email, sender_password, smtp_server, smtp_port, recipient_email, paper_title)
                    email_count += 1
                    print(f"Email sent successfully to {recipient_email} from {sender_email}! {email_count}")
                    break  
                except Exception as e:
                    print(f"Error sending email to {recipient_email} from {sender_email}: {e}")
                    retries -= 1
                    if retries == 0:
                        ...
                        time.sleep(10)
            email_index = (email_index + 1) % len(sender_emails) 
...
send_emails_from_csv(sender_emails, sender_password, smtp_server, smtp_port, csv_filename)
